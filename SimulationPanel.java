...
public class SimulationPanel extends javax.swing.JPanel {
    ...
    public SimulationPanel(PredatorPreySimulator sim, SimulationFrame sFrame) {
        ...
        this.sim = sim;
        this.grid = new CellPanel[sim.numRow][sim.numCol];
        for (int i = 0; i < sim.numRow; i++) {
            for (int j = 0; j < sim.numCol; j++) {
                this.grid[i][j] = new CellPanel();
                this.simPanel.add(this.grid[i][j]);
            }
        }
        this.simPanel.setLayout(new java.awt.GridLayout(sim.numRow, sim.numCol, 0, 0));
    ...
    void setLabels() {
        ...
        this.antsLabel.setText("Number of ants: " + this.sim.numAnt);
        this.bugsLabel.setText("Number of bugs: " + this.sim.numBug);
    }
    ...
    private void NextButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_NextButtonActionPerformed
        this.sim.nextStep();
        setLabels();
        for (int i = 0; i < this.sim.numRow; i++) {
            for (int j = 0; j < this.sim.numCol; j++) {
                final int row = i;
                final int col = j;
